...
echo "Mid,$requestNumber,10" >> ./TestResultInfo/TestDescriptor.csv
...
while [ $(expr $(date "+%s") - $startTime) -le $totalTestLength ]; do
    ...
    cd ..
    cd Optimizer
    python3 optimizer_ui.py -n ../kubernetes_delegation/nodes.yaml -r $latestRequest -s ../TestingBaseline/services.yaml -rp ../TestingBaseline/mid_resource_policies_10n.yaml -cs ../TestingBaseline/container_specs.yaml -p ../TestingBaseline/service_ports.yaml -o ../testing-autogen-serv-kubeconf.yaml --dfo ../TestResultInfo/solution-df-$dateTag.csv
    cd ..
